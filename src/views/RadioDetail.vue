<template>
  <div>
    <SnackBar :snackbar="snackbar" text="投稿しました！" @close-snack-bar="snackbar=false" />
      ...
      <v-row justify="center">
        <v-col cols="12" md="10" lg="8" style="position: relative">
          ...
          <div class="radioHeader">
            <div class="font-weight-black text-h6 d-flex justify-space-between align-baseline px-3">
              <div>
                <span class="mplus orange--text" style="font-size: 50px;">{{ radio.times }}</span>
                回目のターン
              </div>
              <div class="text-subtitle-2 grey--text">
                放送日:  {{ radio.broadCastingDate || radio.createdAt }}
              </div>
            </div>
            <div class="text-center mt-10">
              <v-btn dark color="red" large @click="toggleYoutube">YOUTUBE再生</v-btn>
            </div>
                  ...
                  >
                    ...
                    >
                      <div class="mt-2 ml-2" v-if="isSelected(theme._id)">
                        <v-chip dark color="orange" class="font-weight-black">選択中</v-chip>
                      </div>
                    </v-img>
                    ...
                    <div class="text-caption font-weight-semibold pa-3 ">
                      <p class="text-subtitle-2 font-weight-bold">コーナー説明</p>
                      {{ theme.description }}
                    </div>
                  </v-card>
                </v-hover>
...
export default {
  ...
  setup(props) {
    ...
    const youtubeUrl       = ref("https://www.youtube.com/watch?v=D0X3E0qRrhc")
    const youtubeWindow    = ref(false)

    // apollo state
    const { result: radioResult, loading } = useQuery(GET_RADIO, { radioId: props.id })
    const radio = useResult(radioResult, null, data => data.radio)

    const { result: radioThemeResult, onResult } = useQuery(GET_RADIO_THEMES, { radioId: props.id })
    onResult((data) => {
      selectedThemeId.value = data.data.radioThemes[0]._id
    })
    const themes = useResult(radioThemeResult, null, data => data.radioThemes)
    // const themeTitles = useResult(radioThemeResult, null, data => data.radioThemes.map(theme => theme.title))
